...
*   **Experience-by-Experience Map-Reduce Tailoring**: Processes and tailors each selected professional experience entry in complete isolation (**Map Phase**). By extracting organization-specific slugs, the engine matches and injects only the patents, projects, and performance notes related to that specific company block. This reduces LLM context window size and prevents context dilution, maximizing the model's focus. The individual tailored blocks are then sequentially reduced and assembled into a complete resume layout (**Reduce Phase**), appending education and spoken languages at the end.
